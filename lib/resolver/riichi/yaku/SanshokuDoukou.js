...
export const SanshokuDoukou = {
    ...
    checker: groups => {
        ...
        if (triplesQuads.length < 3)
            return false;
        const firstTileCodes = triplesQuads
            .map(g => g.tiles[0].tile.code)
            .sort((a, b) => a - b);
        // case: The three first triples/quads do sanshoku [TQ1, TQ2, TQ3, X, D]
        if (firstTileCodes[1] === firstTileCodes[0] + 10 && firstTileCodes[2] === firstTileCodes[0] + 20)
            return true;
        if (firstTileCodes.length === 3)
            return false;
...
if (import.meta.vitest) {
    ...
    test("Sanshoku Doukou", () => {
        ...
        const result = SanshokuDoukou.checker(groups);
