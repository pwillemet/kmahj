...
export const KokushiMusou = {
    name: "Kokushi musou",
    checker: (tiles) => {
        if (tiles.length !== MINIMUM_FULL_HAND_SIZE)
            return false;
        if (tiles.some(t => t.modificators.some(m => m !== MahjongTileModificator.WINNING)))
            return false;
        if (!tiles.every(t => t.tile.family.category === TILE_FAMILY_CATEGORY.honor || isTerminal(t)))
            return false;
