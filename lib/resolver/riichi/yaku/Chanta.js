...
export const Chanta = {
    name: "Chanta",
    checker: (groups) => {
        const terminalOrHonorEverywhere = groups.every((group) => group.tiles.some(tile => tile.tile.family.category === TILE_FAMILY_CATEGORY.honor || isTerminal(tile)));
        if (!terminalOrHonorEverywhere)
            return false;
...
if (import.meta.vitest) {
    ...
    test("Chanta", () => {
        ...
        const result = Chanta.checker(groups);
