...
export const Pinfu = {
    ...
    checker: (groups, roundProps) => {
        // Closed groups
        if (groups.some(g => g.state === MahjongGroupState.OPEN))
            return false;
        ...
        if (doubleTile.family === TILE_FAMILY.dragon)
            return false;
        const doubleTileCode = doubleTile.code;
        if (doubleTileCode === roundProps?.playerWind || doubleTileCode === roundProps?.roundWind)
            return false;
        // No Fu with winning tile
        if (double.tiles.some(t => isWinningTile(t)))
            return false; // tanki
        ...
        if (winningTileIndex === 0 && winningTileNumber === 7)
            return false; // penchan 789
        if (winningTileIndex === 2 && winningTileNumber === 3)
            return false; // penchan 123
        return true;
    }
};
if (import.meta.vitest) {
    ...
    test("Pinfu", () => {
        ...
        const result = Pinfu.checker(groups);
