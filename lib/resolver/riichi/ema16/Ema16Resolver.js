import { ResolverPossibility } from '../../Resolver.model';
import { simpleParsedGroupToMahjongGroup } from './utils';
import { isChiiToitsuHandPattern, isClassicHandPattern, KokushiMusou, Pinfu } from '../yaku';
import { WINNING_KIND } from '../riichi.model';
import { GroupType, isOrdinary, MahjongGroupState, MahjongTileModificator, TILE_FAMILY } from '../../../core';
import { EMA16_CLASSIC_YAKUMAN, EMA16_MIXED_YAKUMAN, EMA16_NORMAL_CHII_TOITSU_YAKU, EMA16_NORMAL_CLASSIC_YAKU, EMA16_NORMAL_MIXED_YAKU } from './yaku';
import { RiichiScoreCalculator } from '../ScoreCalculator';
import { roundUp } from '../../../global/math';
export const Ema16Resolver = (parsedHand, roundProps) => {
    if (parsedHand.groups.length === 0)
        return null;
    const mahjongHandWithoutScore = getMahjongHandWithoutScore(parsedHand, roundProps);
    if (mahjongHandWithoutScore == null)
        return null;
...
function getMahjongHandWithoutScore(parsedHand, roundProps) {
    ...
    for (let chiiToitsuHandPossibility of chiiToitsuHandPossibilities) {
        const yakuman = EMA16_MIXED_YAKUMAN.find(y => y.checker(chiiToitsuHandPossibility.groups, roundProps));
        if (yakuman != null)
            return {
    ...
    for (let classicHandPossibility of classicHandPossibilities) {
        const yakuman = [...EMA16_CLASSIC_YAKUMAN, ...EMA16_MIXED_YAKUMAN].find(y => y.checker(classicHandPossibility.groups, roundProps));
        if (yakuman != null)
            return {
    ...
    const hands = [];
    for (let chiiToitsuHandPossibility of chiiToitsuHandPossibilities) {
        const yakus = [EMA16_NORMAL_CHII_TOITSU_YAKU, ...EMA16_NORMAL_MIXED_YAKU].filter(y => y.checker(chiiToitsuHandPossibility.groups, roundProps));
        if (yakus.length > 0) {
    ...
    for (let classicHandPossibility of classicHandPossibilities) {
        const yakus = [...EMA16_NORMAL_CLASSIC_YAKU, ...EMA16_NORMAL_MIXED_YAKU].filter(y => y.checker(classicHandPossibility.groups, roundProps));
        if (yakus.length > 0) {
            ...
            hands.push({
                ...
                han,
                yakuman: 0,
                fu: countFu(classicHandPossibility.groups, yakus.map(y => y.name), roundProps),
                yaku: yakus
            });
        }
    }
...
function countFu(groups, yakus, roundProps) {
    ...
    if (total === 20 && groups.some(g => g.state === MahjongGroupState.OPEN))
        total += 2; // Open Pinfu
