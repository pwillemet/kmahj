...
import { GroupType, MahjongGroup, MahjongGroupState, MahjongTile, MahjongTileModificator } from '../../../core'
import { ResolverError } from '../../error'

export function simpleParsedGroupToMahjongGroup({ tiles }: ParsedGroup): MahjongGroup {
  ...
  if (isQuad(tiles)) return {
    tiles,
    state,
    isolated: true,
    type: GroupType.QUAD,
  };
  if (isTriple(tiles)) return {
    tiles,
    state,
    isolated: true,
    type: GroupType.TRIPLE
  }
  if (isDouble(tiles)) return {
    tiles,
    state,
    isolated: true,
...
export function isQuad(tiles: MahjongTile[]): boolean {
  return tiles.length === 4 &&
    tiles.every(t => t.tile.code === tiles[0].tile.code)
}


export function isTriple(tiles: MahjongTile[]): boolean {
  return tiles.length === 3 && tiles.every(t => t.tile.code === tiles[0].tile.code);
}

export function isDouble(tiles: MahjongTile[]): boolean {
  return tiles.length === 2 && tiles[0].tile.code === tiles[1].tile.code;
}

export function isSequence(tiles:MahjongTile[]): boolean {
